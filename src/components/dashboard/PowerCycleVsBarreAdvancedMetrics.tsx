...
export const PowerCycleVsBarreAdvancedMetrics: React.FC<PowerCycleVsBarreAdvancedMetricsProps> = ({ data }) => {
  const processedData = useMemo(() => {
    ...
    const timeAnalysis = data.reduce((acc, session) => {
      ...
      if (isPowerCycle) {
        acc[timeSlot].powercycle += session.checkedIn;
        acc[timeSlot].pcAttendance += session.checkedIn;
        acc[timeSlot].pcSessions += 1;
      } else if (isBarre) {
        acc[timeSlot].barre += session.checkedIn;
        acc[timeSlot].barreAttendance += session.checkedIn;
    ...
    const locationAnalysis = data.reduce((acc, session) => {
      ...
      if (isPowerCycle) {
        ...
        acc[location].pcAttendance += session.checkedIn;
        acc[location].pcCapacity += session.capacity;
      } else if (isBarre) {
        acc[location].barreSessions += 1;
        acc[location].barreAttendance += session.checkedIn;
