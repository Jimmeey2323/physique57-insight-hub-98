...
import { Calendar, Filter, RotateCcw, Search, Users, MapPin, User, Clock } from 'lucide-react';
...
export const RecurringClassFilterSection: React.FC<RecurringClassFilterSectionProps> = ({
  ...
  const getTimeSlot = (time: string): string => {
    ...
    if (hour >= 6 && hour < 9) return 'Early Morning (6:00-9:00)';
    if (hour >= 9 && hour < 12) return 'Morning (9:00-12:00)';
    if (hour >= 12 && hour < 17) return 'Afternoon (12:00-17:00)';
    if (hour >= 17 && hour < 20) return 'Evening (17:00-20:00)';
    if (hour >= 20 && hour < 23) return 'Night (20:00-23:00)';
    return '';
  };
